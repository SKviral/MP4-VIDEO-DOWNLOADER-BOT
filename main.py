...
# ══════════════════════════════════════════════════════════════════════
# State tracking: { user_id: [{"message": ..., "handler_type": ..., "status_msg": ..., "url": ...}] }
user_queues: dict[int, list] = {}
user_processing: dict[int, bool] = {}

async def update_queue_messages(user_id: int):
    items = user_queues.get(user_id, [])
...
async def process_next_queue_item(user_id: int, client, item: dict):
    ...
    try:
        ...
        queue_list = user_queues.get(user_id, [])
        if queue_list:
            next_item = queue_list.pop(0)
            await update_queue_messages(user_id)
            asyncio.create_task(process_next_queue_item(user_id, client, next_item))
        else:
...
async def queue_message(client, message: Message, handler_type: str):
    ...
    url = ""
    if handler_type == "media":
        urls = extract_all_terabox_urls(message.caption or "")
        url = ", ".join(urls) if urls else "Media Link"
    elif message.text:
    ...
    is_busy = user_processing.get(user_id, False)
    
    if is_busy:
        pos = len(user_queues[user_id]) + 1
        ...
        asyncio.create_task(process_next_queue_item(user_id, client, {
            "message": message,
            "handler_type": handler_type,
            "status_msg": None,
            "url": url
...
# DOWNLOAD ENGINE
# ══════════════════════════════════════════════════════════════════════
async def download_file(url: str, save_path: str, status_msg, speed_tracking: bool = True, original_url: str = None):
        ...
        if attempt < MAX_RETRIES:
            try:
                await status_msg.edit_text(
                    f"{prefix}⚠️ চেষ্টা {attempt}/{MAX_RETRIES} ব্যর্থ: {err}\n🔄 আবার চেষ্টা করছি..."
                )
            except Exception:
                pass
...
async def download_terabox_with_fallback(
    primary_url: str, fallback_url: str | None, save_path: str, status_msg, original_url: str = None
):
    ok, result = await download_file(primary_url, save_path, status_msg, original_url=original_url)
    if ok:
        return True, result
    if fallback_url and fallback_url != primary_url:
        ...
        try:
            prefix = f"🔗 লিংক: {original_url}\n\n" if original_url else ""
            await status_msg.edit_text(f"{prefix}⚠️ মূল লিংক ব্যর্থ, বিকল্প লিংক দিয়ে চেষ্টা হচ্ছে...")
        except Exception:
            pass
        if os.path.exists(save_path):
            os.remove(save_path)
        ok2, result2 = await download_file(fallback_url, save_path, status_msg, original_url=original_url)
        if ok2:
            return True, result2
...
async def send_video_file(
    ...
    prefix = f"🔗 লিংক: {original_url}\n\n" if original_url else ""
    try:
        await status_msg.edit_text(f"{prefix}🔧 ভিডিও প্রসেস করা হচ্ছে...")
    except Exception:
        pass
    ...
    try:
        await status_msg.edit_text(
            f"{prefix}✅ প্রসেস সম্পন্ন ({size_mb:.2f} MB)।\n⬆️ আপলোড হচ্ছে... 🚀"
        )
    except Exception:
        pass
...
async def post_to_channels(
    ...
    downloaded_videos: list[dict] = None,
):
    """
    চ্যানেলে পোস্টের অর্ডার:
    ১. সব ডাউনলোড করা Terabox ভিডিও (VIDEO FIRST)
    ২. অরিজিনাল মিডিয়া (photo/video) — IMAGE LAST (থাম্বনেইল)
    """
    success_count = 0
    fail_msgs = []
    ...
    for ch in channels:
        ...
        for attempt in range(1, 3):
            try:
                # ─── ১. প্রথমে ডাউনলোড করা সবগুলো ভিডিও পাঠাও ───────────────────────
                if downloaded_videos:
                    for vid_info in downloaded_videos:
                        vid_path = vid_info["path"]
                        vid_name = vid_info["name"]
                        if os.path.exists(vid_path):
                            vid_path = await ensure_playable_video(vid_path)
...
async def handle_forwarded_media(client, message: Message):
    ...
    terabox_urls = extract_all_terabox_urls(caption)
    if not terabox_urls:
...
async def actual_handle_forwarded_media(client, message: Message, status_msg=None):
    ...
    display_urls = ", ".join(terabox_urls)
    ...
    msg_text = (
        f"🔗 লিংক: {display_urls}\n\n"
        f"📢 Terabox পোস্ট শনাক্ত হয়েছে! ({len(terabox_urls)}টি লিংক)\n"
    ...
    all_files_info = []
    for u_idx, t_url in enumerate(terabox_urls, 1):
        try:
            await status_msg.edit_text(
                f"🔗 লিংক: {display_urls}\n\n"
            ...
            if not ok:
                await status_msg.edit_text(
                    f"🔗 লিংক: {display_urls}\n\n"
                    f"❌ Terabox ডাউনলোড ব্যর্থ (লিংক {u_idx})!\n{info}\n\n"
                    "💡 নিজের API Key যোগ করতে /api দিন।"
                )
                continue
            
            for file_info in info:
                file_info["original_url"] = t_url
                all_files_info.append(file_info)
        except Exception as e:
            print(f"Error fetching info for link {t_url}: {e}", flush=True)

    if not all_files_info:
        await status_msg.edit_text(
            f"🔗 লিংক: {display_urls}\n\n"
            f"❌ কোনো ভিডিও লিংক সংগ্রহ করা সম্ভব হয়নি।"
        )
        return
    ...
    # ২. সব ফাইল একে একে ডাউনলোড করো
    for i, file_info in enumerate(all_files_info, 1):
        dl_url    = file_info["download_url"]
        zip_url   = file_info.get("zip_url")
        file_name = file_info["file_name"]
        t_url     = file_info["original_url"]
        ...
        try:
            ...
            success, result = await download_terabox_with_fallback(
                dl_url, zip_url, dl_path, status_msg, original_url=t_url
            )
            if not success:
                await status_msg.edit_text(f"🔗 লিংক: {display_urls}\n\n❌ ডাউনলোড ব্যর্থ: {result}")
                all_failures.append(f"❌ ডাউনলোড ব্যর্থ: {file_name} ({result})")
                continue

            fixed_path = await ensure_playable_video(dl_path)
            downloaded_videos.append({
                "path": fixed_path,
                "name": file_name
            })
        except Exception as e:
            all_failures.append(f"❌ এরর: {file_name} ({str(e)})")
            if os.path.exists(dl_path):
                os.remove(dl_path)

    # ৩. এবার ডাউনলোড হওয়া ভিডিওগুলো এবং থাম্বনেইল চ্যানেলে পোস্ট করো
    if not downloaded_videos:
        err_msg = "\n".join(all_failures) if all_failures else "অজানা সমস্যা।"
        await status_msg.edit_text(
            f"🔗 লিংক: {display_urls}\n\n"
            f"❌ **ডাউনলোড সম্পূর্ণ ব্যর্থ!**\n\n"
            f"ভুলসমূহ:\n{err_msg}"
        )
        return
    ...
    # ৪. টেম্পোরারি ফাইলগুলো ক্লিনআপ করো
    for vid_info in downloaded_videos:
        try:
    ...
    if ok_count == 0:
        ...
        await status_msg.edit_text(
            ...
            f"📢 সফল চ্যানেল: {ok_count}/{len(channels)}\n"
            f"❌ ব্যর্থ হয়েছে:\n{err_msg}"
        )
    else:
        await status_msg.edit_text(
            ...
            f"✅ **সম্পন্ন!**\n\n"
            f"📢 চ্যানেল: {ch_names}\n"
            f"🎬 পোস্ট করা হয়েছে: {len(downloaded_videos)}টি ভিডিও"
        )
    ...
    try:
        ...
        if is_terabox_url(url):
            ...
            for i, file_info in enumerate(info, 1):
                ...
                success, result = await download_terabox_with_fallback(
                    dl_url, zip_url, dl_path, status_msg, original_url=url
                )
                if not success:
                    await status_msg.edit_text(f"🔗 লিংক: {url}\n\n❌ ডাউনলোড ব্যর্থ ({file_name})!\n{result}")
                    continue
        ...
        # ── সাধারণ লিংক ──────────────────────────────────────────
        await status_msg.edit_text(f"🔗 লিংক: {url}\n⬇️ ডাউনলোড শুরু হচ্ছে...")
        success, result = await download_file(url, temp_dl, status_msg, original_url=url)
        ...
        if is_zip:
            ...
            await status_msg.edit_text(f"🔗 লিংক: {url}\n📦 ZIP ফাইল পাওয়া গেছে। আনজিপ করা হচ্ছে...")
            try:
                with zipfile.ZipFile(zip_path, "r") as zf:
                    names = zf.namelist()
                ...
                await status_msg.edit_text(f"🔗 লিংক: {url}\n❌ ফাইলটি সঠিক ZIP ফরম্যাটে নেই।")
                return

            videos = find_videos_in_zip(temp_dir)
            if not videos:
                await status_msg.edit_text(f"🔗 লিংক: {url}\n❌ ZIP আনজিপ করার পরেও কোনো ভিডিও পাওয়া যায়নি।")
                return
            ...
            for i, (video_path, video_fname) in enumerate(videos, 1):
                ...
                try:
                    await status_msg.edit_text(
                        f"🔗 লিংক: {url}\n🔧 প্রসেস হচ্ছে ({i}/{total_videos}): {video_fname}"
                    )
                except Exception:
                    pass
                # ভিডিও রিপেয়ার
